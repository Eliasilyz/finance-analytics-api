...
9. **GET /api/v1/stocks/:symbol/technical** -> 200 `{"data":{"symbol","last_date","values":{"daily_return","sma20","sma50","sma200","ema20","rsi14","volatility20","average_volume20"}}}` (nilai terbaru tiap indikator). **POLA PARTIAL-NULL, sama seperti /analytics**: tiap indikator dihitung independen dari data yang tersedia; field yang datanya kurang dari window-nya -> `null` (mis. history 60 hari -> sma200:null, tapi sma20/sma50/rsi14/daily_return semuanya terisi). Constraint per field: daily_return>=2, sma/ema/avg_vol>=window, rsi>=period+1, volatility>=window+1. **422 INSUFFICIENT_DATA HANYA jika history benar-benar 0 baris.** 404 company tak ada.
10. **GET /api/v1/compare?symbols=AAPL,MSFT** -> 200 `{"data":[{"symbol","metrics":{...}}]}`. Tiap simbol dihitung pakai alur analytics yang sama. **Keputusan: ALL-OR-NOTHING, bukan partial success.** Bila SATU saja simbol format-invalid / >10 / duplikat -> 400 utk SELURUH request (message menyebut simbol pertama yang bermasalah); ada simbol tak ditemukan -> 404; ada simbol tanpa data cukup -> 422. Bila semua valid & ditemukan -> 200 seluruh array.
    - Alasan: konsisten dengan endpoint single-symbol (yang juga 400/404/422 utk kegagalan) dan menghindari partial-success yang menyembunyikan error — konsumen tak perlu menebak apakah tiap item punya makna valid. Strict validation (user instruction: validasi tiap simbol) berarti request invalid = request invalid, bukan diinterpretasikan.
11. **GET /api/v1/screener?min_roe=0.15&max_pe=30&sector=Technology** -> filtering di SQL (section 8 INTRUCT): 200 `{"data":[{"symbol","name","sector","roe","pe"}]}`. Param opsional `sector` (eksak), `min_roe`, `max_pe`, `min_revenue_growth`; invalid numeric -> 400; company ditemukan oleh kriteria -> 200 (bisa kosong). **WAJIB parameterized query (placeholder $1..$N via QueryRow/ExecContext), SAMA SEKALI TIDAK string concatenation / fmt.Sprintf untuk nilai filter** — endpoint paling rawan SQL injection karena banyak filter dinamis; bulid WHERE hanya sebagai `AND col $op $n` + slice args, nilainya selalu lewat placeholder.
